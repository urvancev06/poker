...
- **Calldown should scale with made-hand strength (BOT realism).** `bots/strategy.py` folds *any* one-pair hand to a bet at a flat `calldown_freq` (top pair treated the same as bottom pair), so the bots over-fold made hands postflop. The 8k volume stress caught the TAG hero folding **82%-equity top pair** and an overpair (AA) at 54%. It's shared code, so **every archetype has a version** — the tight regs over-fold the most (Nit 0.30, TAG 0.36 → they fold ~64–70% of one-pair hands to a single bet). This degrades *practice*, not just commentary: opponents that fold too much make c-bets/barrels work too often and under-pay value. Fix: grade the calldown by tier (top pair / overpair continue far more than bottom pair) instead of a flat frequency.
- **No implied / reverse-implied odds (coach).** The price is judged on direct pot odds only, so set-mining a small pair reads "close," not the clear call it is (e.g. 22 vs a TAG open: ~43% equity / ~40% required → "close"), and dominated draws aren't penalised enough. Fix: an implied-odds term on the *price* (`required = call / (pot + call + X)`), leaving the conditioned range untouched.
